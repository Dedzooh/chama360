...
const localApkPath = new URL(`./public/downloads/chama360-${packageJson.version}.apk`, import.meta.url);

const localApkDownload = {
  name: 'chama360-local-apk-download',
  configureServer(server: { middlewares: { use: (path: string, handler: (req: import('node:http').IncomingMessage, res: import('node:http').ServerResponse, next: () => void) => void) => void } }) {
    server.middlewares.use(`/downloads/chama360-${packageJson.version}.apk`, (req, res, next) => {
      if (!existsSync(localApkPath)) return next();
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/vnd.android.package-archive');
      res.setHeader('Content-Disposition', `attachment; filename="chama360-${packageJson.version}.apk"`);
