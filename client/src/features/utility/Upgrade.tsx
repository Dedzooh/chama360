...
export const Upgrade = () => {
  ...
  return <div className="subscription-page">
    <section className="subscription-hero"><span><Crown /> CHAMA360 plans</span><h1>Choose the operating package your organization needs</h1><p>Start with practical Chama operations, then upgrade for records, governance, automation, or a tailored enterprise engagement. Annual billing saves roughly two months.</p></section>
    ...
    <section className="subscription-grid">{SUBSCRIPTION_PLANS.map((plan) => {
      ...
      return <article key={plan.id} className={plan.popular ? 'is-popular' : ''}>{plan.popular ? <span className="subscription-popular">Recommended</span> : null}<h2>{plan.name}</h2><p>{plan.description}</p>{plan.id === 'ENTERPRISE' ? <strong>Tailored quote<small> setup + subscription</small></strong> : <><strong>KES {price.toLocaleString()}<small>{plan.monthlyPrice ? (state.billingCycle === 'ANNUAL' ? '/year' : '/month') : ' forever free'}</small></strong>{state.billingCycle === 'ANNUAL' && annualSavings ? <em className="subscription-savings">Save KES {annualSavings.toLocaleString()} · almost 2 months free</em> : null}</>}<ul>{plan.features.map((item) => <li key={item}><Check /> {item}</li>)}<li><Check /> Storage: {plan.storage.label}</li></ul><button type="button" disabled={!state.organizationId || state.loading || (state.plan === plan.id && (state.daysRemaining === null || state.daysRemaining > 7)) || plan.id === 'FREE'} onClick={() => { if (plan.id === 'ENTERPRISE') { setCustomOpen(true); return; } void api.post('/subscriptions/funnel-events', { eventType: 'CHECKOUT_STARTED', organizationId: state.organizationId || undefined, plan: plan.id, billingCycle: state.billingCycle }).catch(() => undefined); void state.requestPlan(plan.id); }}>{plan.id === 'ENTERPRISE' ? 'Contact CHAMA360' : state.plan === plan.id ? (state.daysRemaining !== null && state.daysRemaining <= 7 ? `Renew ${plan.name}` : 'Current plan') : `Choose ${plan.name}`}</button></article>;
    })}</section>
    ...
    <section className="section-shell p-5" aria-labelledby="services-title">
      ...
      <div className="mt-5 rounded-[var(--ds-radius-lg)] bg-[var(--ds-surface-2)] p-4"><strong className="text-sm text-[var(--ds-secondary)]">Refer a Chama</strong><p className="mt-1 text-sm text-[var(--ds-text-muted)]">Referral rewards can be tracked against annual paid conversions. Contact CHAMA360 to register a referral and eligibility.</p></div>
    </section>
