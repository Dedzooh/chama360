...
export const Upgrade = () => {
  ...
  const selectedPlan = SUBSCRIPTION_PLANS.find((plan) => plan.id === state.pendingPlan);
  const selectedPrice = state.billingCycle === 'ANNUAL' ? selectedPlan?.annualPrice : selectedPlan?.monthlyPrice;
  ...
  return <div className="subscription-page">
    <section className="subscription-hero"><span><Crown /> CHAMA360 plans</span><h1>Choose the tools your chama needs</h1><p>One subscription covers the whole chama. Choose monthly flexibility or save two months with annual billing.</p></section>
    <section className="subscription-organization-picker" aria-labelledby="billing-organization-title">
      ...
      <div>
        ...
        <small>Only Chamas where you are an administrator or treasurer are listed.</small>
      </div>
      <label>
      ...
      {selectedOrganization ? <div className="subscription-organization-selection"><Check /><span><strong>{selectedOrganization.name}</strong><small>Currently on {selectedOrganization.subscription.plan} · Purchase will apply to this Chama</small></span></div> : null}
    ...
    <section className="subscription-grid">{SUBSCRIPTION_PLANS.map((plan) => {
      const price = state.billingCycle === 'ANNUAL' ? plan.annualPrice : plan.monthlyPrice;
      return <article key={plan.id} className={plan.popular ? 'is-popular' : ''}>{plan.popular ? <span className="subscription-popular">Recommended</span> : null}<h2>{plan.name}</h2><p>{plan.description}</p>{plan.id === 'ENTERPRISE' ? <strong>Tailored quote<small> based on requirements</small></strong> : <strong>KES {price.toLocaleString()}<small>{plan.monthlyPrice ? (state.billingCycle === 'ANNUAL' ? '/year' : '/month') : ' trial'}</small></strong>}<ul>{plan.features.map((item) => <li key={item}><Check /> {item}</li>)}</ul><button type="button" disabled={!state.organizationId || state.loading || (state.plan === plan.id && (state.daysRemaining === null || state.daysRemaining > 7)) || plan.id === 'FREE'} onClick={() => plan.id === 'ENTERPRISE' ? setCustomOpen(true) : void state.requestPlan(plan.id)}>{plan.id === 'ENTERPRISE' ? 'Contact CHAMA360' : state.plan === plan.id ? (state.daysRemaining !== null && state.daysRemaining <= 7 ? `Renew ${plan.name}` : 'Current plan') : `Choose ${plan.name}`}</button></article>;
    })}</section>
